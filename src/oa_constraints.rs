...
impl<const N: usize, const MOLS: usize> OAConstraints<N, MOLS> {
    ...
    fn get_value_pair(
        ...
        (self.oa.get(min, i, j), self.oa.get(max, i, j))
    }
    ...
    fn set(&mut self, column: usize, index: usize, value: usize) {
        ...
        self.oa.set(column, row, col, Some(value as u8));
        ...
        while changed {
            ...
            for first_column in 0..MOLS {
                for second_column in (first_column + 1)..MOLS {
                    ...
                    {
                        let Cell(i, j) = Cell::from_index::<N>(index);
                        let first_value = self.oa.get(first_column, i, j).unwrap() as usize;
                    ...
                    {
                        let Cell(i, j) = Cell::from_index::<N>(index);
                        let second_value = self.oa.get(second_column, i, j).unwrap() as usize;
            ...
            // for col in (1..(MOLS)).rev() {
            //     let prev_col = col - 1;

            //     let Some(max_val) = self.values_for_cell(col, N).into_iter().last() else {
            //         continue;
            //     };

            //     let mask = SmallBitSet::all_less_than(max_val);

            //     let values = &mut self.cell_values[prev_col][1][0];
            //     *values = values.intersect(mask);
            // }
            // for col in 0..(MOLS - 1) {
            //     let next_col = col + 1;

            //     let Some(min_val) = self.values_for_cell(col, N).into_iter().next() else {
            //         continue;
            //     };
    ...
    pub fn cells_for_pair(
        ...
        for i in 0..N {
            for j in 0..N {
                first_values[i * N + j] = if let Some(val) = self.oa.get(min, i, j) {
        ...
        for i in 0..N {
            for j in 0..N {
                second_values[i * N + j] = if let Some(val) = self.oa.get(max, i, j) {
    ...
    pub fn pairs_for_cell(&self, column1: usize, column2: usize, index: usize) -> BigBitSet {
        ...
        let values1 = if let Some(v1) = self.oa.get(min, i, j) {
            SmallBitSet::single(v1 as usize)
        } else {
            self.values_for_cell(min, index)
        };
        let values2 = if let Some(v2) = self.oa.get(max, i, j) {
    ...
    pub fn find_and_set_singles(&mut self) {
        ...
        while changed {
            changed = false;
            // set cells with only one possibility
            for column in 0..MOLS {
                for index in self.empty_cells[column] {
            ...
            for column in 0..MOLS {
                for i in 0..N {
                    // rows
                    let mut candidates = self.rows[column][i];
                    ...
                    for value in candidates {
                        for j in 0..N {
                            if self.empty_cells[column].contains(i * N + j)
                                && self.values_for_cell(column, i * N + j).contains(value)
                            {
                                self.set_and_propagate(column, i * N + j, value);
                                changed = true;
                                break;
                            }
                        }
                    ...
                    // columns
                    let mut candidates = self.columns[column][i];
                    let mut found = BitSet16::empty();
                    ...
                    for value in candidates {
                        for j in 0..N {
                            if self.empty_cells[column].contains(j * N + i)
                                && self.values_for_cell(column, j * N + i).contains(value)
                            {
                                self.set_and_propagate(column, j * N + i, value);
                                changed = true;
                                break;
                            }
                        }
            ...
            for column1 in 0..MOLS {
                for column2 in (column1 + 1)..MOLS {
                    let pairs = self.get_column_pair_values(column1, column2);

                    for value_pair in *pairs {
                        let value_pair = ValuePair::from_index::<N>(value_pair);
